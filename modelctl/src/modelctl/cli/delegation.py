...
from ..delegation.orchestrator import batch_graph, execute_graph
...
@click.option(
    ...
    "model_ref",
    required=False,
    help="override model (provider/id); allowlisted providers auto-admit",
)
...
@click.option(
    ...
    "model_ref",
    required=False,
    help="override model for every task in the batch; allowlisted providers auto-admit",
)
@pass_ctx
def delegate_batch(ctx: Ctx, role, tasks_dir, model_ref):
    try:
        ...
        if not tasks_path.is_dir():
            raise ModelctlError(
                code="E_CONFIG_INVALID", message=f"tasks-dir not found: {tasks_dir}"
            )
        ...
        for tf in task_files:
            ...
            task["role"] = role
            tasks.append((str(tf), task))
        payload = execute_graph(
            registry=reg,
            config=cfg,
            graph=batch_graph(tasks, role),
            requested_model=model_ref,
            trace_id=ctx.trace_id,
        )
        payload.update({"role": role, "batch_size": len(tasks)})
        emit(payload, ctx)
    except Exception as e:
        handle_error(e, ctx)
...
@click.option(
    ...
    "model_ref",
    required=False,
    help="override model for nodes without an explicit model",
)
@pass_ctx
def delegate_graph(ctx: Ctx, graph_file, model_ref):
    try:
        p = Path(graph_file)
        if not p.exists():
            raise ModelctlError(
                code="E_CONFIG_INVALID", message=f"graph file not found: {graph_file}"
            )
        data = json.loads(p.read_text())
        if not isinstance(data, dict):
            raise ModelctlError(
                code="E_CONFIG_INVALID", message="graph file must contain a JSON object"
            )
        cfg, reg = load_pair(ctx)
        emit(
            execute_graph(
...
@click.option(
    "--bin", "bin_", required=False, type=click.Choice(["worker", "driver", "unclassified"])
)
@pass_ctx
...
def delegates_show(ctx: Ctx, model_ref):
    try:
        ...
        if not m:
            raise ModelctlError(
                code="E_DELEGATE_MODEL_UNAVAILABLE", message=f"model not found: {model_ref}"
            )
...
def delegates_doctor(ctx: Ctx):
    try:
        ...
        driver_model = dcfg.driver.model or DEFAULT_MODELS["driver"]
        worker_model = dcfg.worker.model or DEFAULT_MODELS["worker"]

        def _available(ref: str) -> bool:
            return any(
                m["model_ref"] == ref and m["availability_status"] == "AVAILABLE" for m in catalog
            )

        checks = [
            {"check": "opencode_executable", "ok": ok, "detail": detail},
            {
                "check": "opencode_version",
                "ok": version is not None,
                "detail": version or "unknown",
            },
            {
                "check": "driver_model_available",
                "ok": _available(driver_model),
                "detail": driver_model,
            },
            {
                "check": "worker_model_available",
                "ok": _available(worker_model),
                "detail": worker_model,
            },
            {
                "check": "catalog_freshness",
                "ok": len(catalog) > 0,
                "detail": f"{len(catalog)} models",
            },
            {
                "check": "provider_allowlist",
                "ok": True,
                "detail": str(
                    cfg.get("delegation", {})
                    .get("execution", {})
                    .get("provider_allowlist", [DEFAULT_MODELS["driver"].split("/", 1)[0]])
                ),
            },
        ...
        lock_data = {
            ...
            "catalog_digest": hashlib.sha256(
                json.dumps([m["model_ref"] for m in catalog], sort_keys=True).encode()
            ).hexdigest()[:12],
            "brain_authority": cfg.get("delegation", {}).get("brain", {}).get("owner", "codex"),
            "driver_model_ref": driver_model,
            "worker_model_ref": worker_model,
            "default_variant": "model-default",
    ...
    try:
        ...
        if action == "status":
            ...
            raise ModelctlError(
                ...
                message="warm opencode broker is not implemented; refusing to fake a running broker",
            )
        elif action == "stop":
            if broker_state.exists():
                broker_state.unlink()
...
def queue_retry(ctx: Ctx, run_id):
    try:
        cfg, reg = load_pair(ctx)
        envelope = retry_delegate_run(
            registry=reg, config=cfg, run_id=run_id, trace_id=ctx.trace_id
        )
        emit(
            {"ok": True, "new_run_id": envelope["run_id"], "from": run_id, "envelope": envelope},
            ctx,
        )
    except Exception as e:
        handle_error(e, ctx)
        ...
        emit(
            {
                ...
                "soft_daily_usd": soft,
                "hard_daily_usd": hard,
                "remaining_hard": max(0, hard - today) if hard else None,
            },
            ctx,
        )
    except Exception as e:
...
def bench_report(ctx: Ctx):
    try:
        ...
        txt = report(data)
        emit(
            {"ok": True, "report": txt, "data": data, "_render": {"kind": "text", "text": txt}}, ctx
        )
    except Exception as e:
        handle_error(e, ctx)
